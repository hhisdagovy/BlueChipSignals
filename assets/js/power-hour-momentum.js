class PowerHourMomentum {
    ...
    async fetchMarketData() {
        try {
            // Get current snapshot with Polygon
            const snapshotResponse = await fetch(`https://api.polygon.io/v2/snapshot/locale/us/markets/stocks/tickers/${this.symbol}?apiKey=${this.apiKey}`);
            ...
            // Get intraday aggregates (5-minute bars for today)
            const today = new Date();
            const todayStr = today.toISOString().split('T')[0];
            const aggregatesResponse = await fetch(`https://api.polygon.io/v2/aggs/ticker/${this.symbol}/range/5/minute/${todayStr}/${todayStr}?adjusted=true&sort=asc&limit=120&apiKey=${this.apiKey}`);
            const aggregatesData = await aggregatesResponse.json();
            
            return {
                currentPrice: ticker.lastTrade?.p || day.c,
                previousClose: prevDay.c,
                dayHigh: day.h,
                dayLow: day.l,
                openPrice: day.o,
                volume: day.v || 0,
                intraday: aggregatesData.results || []
            };
        } catch (error) {
    ...
    calculateVWAP(intradayData) {
        ...
        for (let i = 0; i < intradayData.length; i++) {
            const bar = intradayData[i];
            const price = (bar.h + bar.l + bar.c) / 3; // Typical price
            const volume = bar.v || 1000; // Fallback volume
    ...
    analyzeTrendStrength(intradayData, currentPrice, openPrice) {
        ...
        const prices = intradayData.map(bar => bar.c);
    ...
    async updateMomentumIndicator() {
        try {
            ...
            if (!marketData || !marketData.intraday || marketData.intraday.length === 0) {
            ...
            const vwapPosition = {
                position: vwap ? (marketData.currentPrice > vwap ? 'Above VWAP' : 'Below VWAP') : 'N/A',
